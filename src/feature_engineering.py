...
class FeatureCreation:
    ...
    def create_features(self):
        try:
            ...
            self.df['intensity'] = (self.df['avg_bpm'] - self.df['resting_bpm']) / self.df['hrr'] # trenirovqada yurak qanchalik kuch bilan ishlaganini, yani butun kuchingdan necha % ishlatkani
            logger.info("Feature 'intensity' created")

            self.df['calories_per_min'] = self.df['calories_burned'] / self.df['session_duration'] # bir minutda qancha kcal yoqatishi
            logger.info("Feature 'calories_per_min' created")

            self.df['water_per_session'] = self.df['water_intake'] / self.df['workout_frequency'] # bir trenirovqada qancha suv ichishi
            logger.info("Feature 'water_per_session' created")

            self.df['experience_intensity_ratio'] = self.df['experience_level'] * self.df['intensity'] # mashqning tajribaga nisbatan qanchalik qiyinligi
